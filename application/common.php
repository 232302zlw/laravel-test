 ...
 */
function fail($font){
    echo json_encode(['msg'=>$font,'code'=>2]);exit;
}
...
function successly($font){
    echo json_encode(['msg'=>$font,'code'=>1]);
}
/**
 * 无限极分类
 ...
 * @return type
 */
function getSonCateInfo($cateInfo,$pid=0,$level=1){
    $info=[];
    foreach($cateInfo as $k=>$v){
        if($v['pid']==$pid){
            $v['level']=$level;
            $info[$k]=$v;
            $info[$k]['son'] = getSonCateInfo($cateInfo,$v['cate_id'],$level+1);
        }
    }
    return $info;
}
...
function sendEmail($address,$subject,$body){
    ...
    $mail->Username ='c18811544302@163.com';

    //smtp登录的密码 使用生成的授权码（就刚才叫你保存的最新的授权码）
    $mail->Password = 'cheng19971210';//163邮箱也有授权码 进入163邮箱帐号获取

    //设置发件人邮箱地址 这里填入上述提到的“发件人邮箱”
    $mail->From = 'c18811544302@163.com';
